...
import {
  AlertCircle, Clock, Loader2, Plus, CheckSquare, Square, Shirt,
} from 'lucide-react';
import type { MatchWithFullDetails, MatchPlayerWithDetails } from '@/lib/types';
import { useMatchDetailStore } from '@/features/matches/presentation/stores/useMatchDetailStore';
...
import { getTeam1Name, getTeam1Logo, getTeam2Name, getTeam2Logo, getTeam1Id, getTeam2Id, getWinnerName, isWinner } from '@/features/matches/domain/usecases/team-helpers';
import TeamLogo from '@/features/matches/presentation/components/TeamLogo';
import { getLeagueName } from '@/features/matches/domain/usecases/league-helpers';
import AddNewPlayerModal from '@/features/player/components/AddNewPlayerModal';
import GameTrackingPanel from '@/features/game-tracking/presentation/components/GameTrackingPanel';
import MatchImagesDisplay from '@/features/matches/presentation/components/MatchImagesDisplay';
import { formatClockTime, getPeriodLabel } from '@/features/game-tracking/domain/usecases/utils';
  ...
  const togglePlayerToAdd = (playerId: string) => {
    setPlayersToAdd((prev) => {
      ...
      if (newMap.has(playerId)) {
        ...
        newMap.set(playerId, { started: true });
      }
      return newMap;
    });
  };
...
export default function MatchDetailView({ matchId, initialMatch }: MatchDetailViewProps) {
  ...
  const {
    match, loading, error,
    showAddPlayerModal, showAddEventModal, showEditPlayerModal,
    editingMatchPlayerId, editEventId,
    deleteEventId, deletePlayerId, deletingPlayer,
    isStartingGame, isEndingGame,
    page1, page2,
  } = store;
  ...
  useEffect(() => {
    if (matchId) {
      store.init(matchId, initialMatch);
    }
  ...
  const isPlayerOnFloor = (mp: MatchPlayerWithDetails) => {
    ...
    if (hasExplicitlyActive) return mp.isActive;
    return match.status === 'LIVE' && mp.started;
  };

  const getPlayersByTeam = (teamId: string) => {
    if (!match?.matchPlayers) return [];
    return [...match.matchPlayers.filter((mp) => mp.teamId === teamId)].sort((a, b) => {
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      {match.status === 'LIVE' || match.status === 'UPCOMING' ? (
        <GameTrackingPanel matchId={matchId} match={match as any} onRefresh={store.fetchMatchDetails} />
      ) : null}
            ...
            <CardHeader>
              ...
              Match Players
                ...
                <Button 
                  size="sm" 
                  variant="outline" 
                  onClick={() => store.setShowAddPlayerModal(true)}
                >
                                ...
                                >
                                  ...
                                  <div className="flex items-center gap-1">
                                    <Button 
                                      ...
                                      size="icon" 
                                      className="h-8 w-8"
                                      onClick={() => store.openEditPlayer(mp.id)}
                                    >
                                    ...
                                    <Button 
                                      ...
                                      onClick={() => store.setDeletePlayerId(mp.id)}
                                    >
                              ...
                              {totalPages > 1 && (
                                <div className="flex items-center justify-center gap-2 mt-4 pt-4 border-t border-dashed">
                                  <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => store.setPage1(Math.max(1, page1 - 1))}
                                    disabled={page1 === 1}
                                  >
                                    Previous
                                  </Button>
                                  ...
                                  <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => store.setPage1(Math.min(totalPages, page1 + 1))}
                              ...
                              {totalPages > 1 && (
                                <div className="flex items-center justify-center gap-2 mt-4 pt-4 border-t border-dashed">
                                  <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => store.setPage2(Math.max(1, page2 - 1))}
                                    disabled={page2 === 1}
                                  >
                                    Previous
                                  </Button>
                                  ...
                                  <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => store.setPage2(Math.min(totalPages, page2 + 1))}
                        ...
                        <div className="flex items-center justify-end mt-2">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem 
                                onClick={() => store.openEditEvent(event.id)}
                                disabled={match?.status !== 'LIVE'}
                              >
                              ...
                              <DropdownMenuItem
                                onClick={() => store.setDeleteEventId(event.id)}
      ...
      {match && (
        <AddPlayerModal
          ...
          onClose={() => store.setShowAddPlayerModal(false)}
          onSuccess={() => {
            store.setShowAddPlayerModal(false);
            store.onPlayerMutationSuccess();
          }}
        />
      )}
      ...
      {match && (
        <EditMatchPlayerModal
          ...
          onClose={store.closeEditPlayer}
          onSuccess={() => {
            store.closeEditPlayer();
            store.fetchMatchDetails();
          }}
        />
      )}
      ...
      <AlertDialog open={deletePlayerId !== null} onOpenChange={(open) => !open && store.setDeletePlayerId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={store.deletePlayer} disabled={deletingPlayer}>
      ...
      <AlertDialog open={deleteEventId !== null} onOpenChange={(open) => !open && store.setDeleteEventId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={store.deleteEvent}>
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
