...
function PodiumCard({ row, val, rank, tag, unit, mode, lead }: { row: LeaderRow; val: string; rank: number; tag: string; unit: string; mode: string; lead: boolean }) {
	...
	return (
		<Tag
			...
			style={{
				background: lead ? "var(--night,#0c0b0a)" : "#fff",
				borderColor: lead ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.1)",
				boxShadow: lead ? "0 20px 40px rgb(var(--site-ink-rgb)/0.18)" : "0 1px 2px rgb(var(--site-ink-rgb)/0.04)",
			}}
		>
			<div className="mb-5 flex items-center justify-between">
				...
				<span
					...
					style={lead ? { background: "rgb(var(--site-brand-rgb) / 0.16)", color: "var(--brandsoft)" } : { background: "rgb(var(--site-brand-rgb) / 0.1)", color: "var(--brand)" }}
				>
					{tag}
				</span>
			</div>
			<div className="flex items-center gap-3">
				<span
					...
					style={{ background: lead ? CREST_DARK : CREST_LIGHT, color: lead ? "var(--brand)" : "var(--ink,#141009)" }}
				>
					{row.initials}
				</span>
				<div className="min-w-0">
					<div className="truncate font-body text-[17px] font-extrabold" style={{ color: lead ? "var(--cream,#f6f2ec)" : "var(--night2,#1a1712)" }}>
						{row.name}
					</div>
					<div className="mt-0.5 font-mono text-[11px]" style={{ color: lead ? "var(--muted2,#8a817a)" : "var(--muted,#6f665c)" }}>
						{row.team}
					</div>
				</div>
			</div>
...
function LeaderRowItem({ row, val, rank }: { row: LeaderRow; val: string; rank: number }) {
	const Tag = row.href && row.href !== "#" ? "a" : "div";
	const rankColor = rank === 1 ? "var(--brand)" : rank <= 3 ? "var(--ink,#141009)" : "var(--muted2,#b3a99c)";
	return (
		<Tag
			{...(Tag === "a" ? { href: row.href } : {})}
