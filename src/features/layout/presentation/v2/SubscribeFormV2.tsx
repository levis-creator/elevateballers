    ...
    <form onSubmit={handleSubmit}>
      <div className="flex gap-2">
        ...
        <button
          type="submit"
          disabled={busy}
          className="cursor-pointer rounded-md border-none bg-brand px-[18px] font-body text-[13px] font-extrabold uppercase text-brandfg disabled:cursor-not-allowed disabled:opacity-60"
        >
      ...
      {message && (
        <p
          className={`mt-1 text-[13px] ${status === 'error' ? 'text-[#ff6b6b]' : 'text-[#5fd08a]'}`}
        >
          {message}
        </p>
      )}
