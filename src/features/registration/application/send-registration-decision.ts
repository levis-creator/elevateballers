...
export async function notifyTeamRegistrationDecision(
  teamId: string,
  approved: boolean
): Promise<void> {
  ...
  if (team.contactEmail)
    recipients.set(team.contactEmail.trim().toLowerCase(), application?.applicantName || team.name);
  if (application?.applicantEmail)
    recipients.set(
      application.applicantEmail.trim().toLowerCase(),
      application.applicantName || team.name
    );
  for (const relation of coaches) {
    if (relation.staff?.email)
      recipients.set(
        relation.staff.email.trim().toLowerCase(),
        `${relation.staff.firstName} ${relation.staff.lastName}`.trim()
      );
  }
  const tasks = [...recipients].map(([email, name]) =>
    approved
      ? application
        ? sendRegistrationApprovedEmail({
            name,
...
export async function notifyPlayerRegistrationDecision(
  playerId: string,
  approved: boolean
): Promise<void> {
