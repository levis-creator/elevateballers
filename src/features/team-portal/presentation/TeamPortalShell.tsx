...
export default function TeamPortalShell({
  ...
  return (
    <div
      className={`team-portal-root min-h-screen w-full bg-night font-body text-cream ${lightMode ? 'portal-light' : ''}`}
    >
        ...
        <aside className="hidden w-[236px] flex-shrink-0 flex-col border-r border-white/[0.06] bg-[#0a0908] min-[900px]:flex">
          <div className="flex items-center gap-2.5 border-b border-white/[0.06] px-4 py-4">
            <span className="rounded-xl bg-white p-1.5">
              <img
                src="/images/Elevate_Icon.png"
                alt="Elevate Ballers Team Portal"
                className="block h-8 w-8 object-contain"
              />
            </span>
            <span className="font-display text-[14px] uppercase tracking-[0.06em] text-cream">
              Team <span className="text-brand">Portal</span>
            </span>
          </div>
          <nav className="flex-1 px-3 py-4" aria-label="Team Portal navigation">
            <div className="mb-2 px-3 font-mono text-[9px] uppercase tracking-[0.18em] text-[#5f574e]">
              My team
            </div>
          ...
          <div className="border-t border-white/[0.06] px-3 py-3.5">
            <a
              href="/admin/profile"
              className="mb-2 block rounded-xl border border-white/[0.08] bg-white/[0.03] px-3 py-2.5 no-underline transition hover:border-brand/50 hover:bg-brand/[0.08]"
            >
              <span className="block font-mono text-[9px] uppercase tracking-[0.16em] text-[#5f574e]">
                Signed in as
              </span>
            ...
            <button
              ...
              className="flex w-full items-center justify-center gap-2 rounded-xl border border-brand/40 bg-brand/[0.12] py-2.5 text-[11px] font-bold uppercase tracking-[0.04em] text-brand hover:bg-brand/[0.2]"
            >
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <header className="sticky top-0 z-30 border-b border-white/[0.06] bg-[#111010] px-4 py-3 min-[900px]:px-7 min-[900px]:py-4">
            <div className="mx-auto flex max-w-[1180px] items-center gap-3">
              <span className="flex h-[38px] w-[38px] flex-shrink-0 items-center justify-center rounded-xl border border-white/[0.08] bg-white/[0.03] font-display text-[15px] text-brand">
                {initials}
              </span>
              <div className="min-w-0 flex-1">
                <div className="truncate font-display text-[16px] uppercase leading-none tracking-[0.03em] text-cream min-[900px]:text-[19px]">
                  ...
                  <label className="mt-1.5 flex w-fit max-w-full items-center gap-2 rounded-xl border border-white/[0.08] bg-white/[0.03] py-1.5 pl-2.5 pr-2">
                    ...
                    >
                      ...
                      {teams
                        ...
                        .map((team) => (
                          <option className="bg-[#111010]" value={team.id} key={team.id}>
                            {team.name}
                          </option>
                        ))}
                    </select>
              ...
              <div className="relative min-[900px]:hidden">
                <button
                  ...
                  className="flex h-9 items-center gap-1.5 rounded-xl border border-white/[0.08] bg-white/[0.03] px-2.5 text-[#b8afa6]"
                >
                  <UserRound size={15} strokeWidth={1.8} />
                  <ChevronDown size={13} strokeWidth={1.8} />
                </button>
                {accountMenuOpen && (
                  <div className="portal-account-dropdown absolute right-0 top-11 z-50 min-w-[150px] rounded-xl border border-white/[0.1] bg-[#171514] p-1.5 shadow-xl">
                    <a
                      href="/admin/profile"
                      className="flex items-center gap-2 rounded-lg px-3 py-2.5 text-[12px] font-semibold text-[#b8afa6] no-underline hover:bg-white/[0.08] hover:text-cream"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => void handleLogout()}
                      className="flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-[12px] font-semibold text-brand hover:bg-brand/[0.12]"
                    >
                      <LogOut size={14} strokeWidth={1.8} />
                      Log out
                    </button>
              ...
              <div className="mx-auto max-w-[1180px]">
                <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
                  <div>
                    ...
                    <h1 className="font-display text-[36px] uppercase leading-none text-cream min-[900px]:text-[44px]">
                      {view === 'overview'
                        ? 'Welcome back'
                        : navigation.find(([route]) => route === view)?.[1]}
                    </h1>
                    <p className="mt-2 max-w-xl text-[13px] leading-relaxed text-[#8a817a]">
                      Your Team Portal workspace is scoped to the active team selected above.
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap items-stretch">
                    <div className="min-w-[290px] flex-1 px-5 py-5">
                      ...
                      <div className="flex items-center gap-3">
                        <span className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/[0.08] bg-white/[0.03] font-display text-[14px] text-[#b8afa6]">
                          —
                        </span>
                      ...
                      <p className="mt-3 text-[13px] text-[#b8afa6]">
                        Fixtures for this active team assignment will appear here when the module is
                        released.
                      </p>
                      <span className="team-scoped-badge mt-4 inline-flex rounded-full bg-white/[0.08] px-3 py-1.5 font-mono text-[10.5px] font-bold uppercase tracking-[0.08em] text-cream">
                        Team scoped
                      </span>
                    </div>
                    <div className="flex min-w-[290px] flex-1 flex-col justify-center gap-3 border-l border-white/[0.06] px-5 py-5 max-[860px]:border-l-0 max-[860px]:border-t">
                      <div className="flex items-center gap-2.5">
                        <span className="flex h-[30px] w-[30px] items-center justify-center rounded-xl bg-[#d99a2b]/[0.16] text-[#d99a2b]">
                        ...
                        <div>
                          ...
                          <div className="font-mono text-[10.5px] text-[#8a817a]">
                            Modules are being prepared for launch
                          </div>
                        </div>
                      </div>
                      <div className="h-[5px] w-full overflow-hidden rounded-full bg-white/[0.08]">
                        <div className="h-full w-1/4 rounded-full bg-[#d99a2b]"></div>
                      </div>
                ...
                <section className="portal-panel mt-4 overflow-hidden rounded-2xl border border-white/[0.08] bg-[#111010]">
                  <div className="flex items-center gap-3 border-b border-white/[0.06] px-5 py-4">
                    <div className="min-w-[200px] flex-1">
                      ...
                      <p className="mt-1 text-[12px] text-[#8a817a]">
                        Actions for your team will appear here.
                      </p>
                    </div>
                  ...
                  <div className="flex flex-col items-center gap-2 px-5 py-9 text-center">
                    ...
                    <div className="text-[13.5px] font-bold text-cream">Nothing waiting on you</div>
                    <div className="max-w-[380px] text-[12px] text-[#8a817a]">
                      Your team-scoped action queue will be available when the module launches.
                    </div>
                  </div>
                </section>
