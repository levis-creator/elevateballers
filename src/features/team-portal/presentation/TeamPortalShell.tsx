...
type PortalView = 'overview' | 'roster' | 'fixtures';
const navigation: Array<[PortalView, string]> = [['overview', 'Overview'], ['roster', 'Roster'], ['fixtures', 'Fixtures']];
...
export default function TeamPortalShell({ name, teams, selectedTeamId, view }: { name: string; teams: Team[]; selectedTeamId: string; view: PortalView }) {
  ...
  return <div className="min-h-screen bg-[#f5f2ed] text-[#181818]">
    ...
    <main className="mx-auto max-w-7xl px-5 py-8 sm:px-8">
      ...
      <nav aria-label="Team Portal" className="mb-8 flex flex-wrap gap-2 border-b border-black/10 pb-4">{navigation.map(([route, label]) => <a key={route} href={`/team-portal?team=${encodeURIComponent(teamId)}&view=${route}`} aria-current={view === route ? 'page' : undefined} className={`rounded-full border px-4 py-2 font-mono text-[11px] uppercase tracking-[0.1em] no-underline ${view === route ? 'border-[#e4002b] bg-[#e4002b] text-white' : 'border-black/10 bg-white text-black/65 hover:border-[#e4002b] hover:text-[#e4002b]'}`}>{label}</a>)}</nav>
      <section className="rounded-2xl border border-black/10 bg-white p-6 shadow-sm sm:p-8"><p className="font-mono text-[10px] uppercase tracking-[0.14em] text-[#e4002b]">{teams.find((team) => team.id === teamId)?.name}</p><div className="mt-3 flex flex-wrap items-center gap-3"><h2 className="font-display text-3xl uppercase">{navigation.find(([route]) => route === view)?.[1]} workspace</h2><span className="rounded-full border border-[#e4002b]/30 bg-[#e4002b]/10 px-2.5 py-1 font-mono text-[10px] uppercase tracking-[0.1em] text-[#e4002b]">Coming soon</span></div><p className="mt-3 max-w-2xl text-sm leading-6 text-black/60">This team-scoped module is planned and will be available here soon. Your access and active team selection are already working.</p><div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{['Roster', 'Fixtures', 'Match sheets', 'Team updates'].map((label) => <div key={label} className="rounded-xl border border-dashed border-black/15 bg-[#f8f6f2] p-4"><div className="font-body font-bold">{label}</div><div className="mt-1 font-mono text-[10px] uppercase tracking-[0.08em] text-black/45">Coming soon</div></div>)}</div></section>
    </main>
  </div>;
}
