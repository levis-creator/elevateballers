    ...
    async (files: File[], refreshCallback: () => void) => {
      ...
      for (const queueItem of queueItems) {
        try {
          ...
          const result = data.results?.find((r: any) => r.title === queueItem.file.name) || data.results?.[0];

          if (!response.ok || !result || result.error || data.successful !== 1) {
            throw new Error(result?.error || data.error || 'The file was not saved to the media database');
          }
