  ...
  try {
    if ('featured' in data && typeof data.featured === 'boolean') {
      ...
      const restData = Object.fromEntries(
        Object.entries(data).filter(([key]) => key !== 'featured')
      ) as UpdateMediaInput;
...
export async function deleteMedia(id: string): Promise<boolean> {
  try {
    ...
    if (media.filePath) {
      try {
        ...
        if (otherWithSameFile.length === 0) {
          const { deleteFile, getStorageTypeForUrl } = await import('../../../../../lib/file-storage');
          const deleted = await deleteFile(media.filePath, getStorageTypeForUrl(media.url));
    ...
    if (media.filePath) {
      try {
        const { fileExists, getStorageTypeForUrl } = await import('../../../../../lib/file-storage');
        const fileStillExists = await fileExists(media.filePath, getStorageTypeForUrl(media.url));
        if (!fileStillExists) {
