...
export default function AdminSidebarV2() {
  ...
  return (
    <aside
      className={`eb-scroll fixed left-0 top-0 z-40 flex h-screen w-[248px] flex-shrink-0 flex-col overflow-y-auto overflow-x-hidden border-r border-[var(--bord2)] bg-[var(--sidebar)] transition-transform duration-[280ms] ease-out ${
        sidebarShown ? 'translate-x-0' : '-translate-x-full'
      }`}
    >
      {/* three-point arc flourish */}
      <svg
        className="pointer-events-none absolute -right-[60px] top-[120px] h-[200px] w-[200px] opacity-40"
        viewBox="0 0 200 200"
        fill="none"
        aria-hidden
      >
        ...
        {ADMIN_NAV.map((group) => {
          ...
          return (
            <div key={group.label} className="mb-4">
              <div className="mb-1.5 px-3 font-['Space_Mono'] text-[9.5px] uppercase tracking-[0.18em] text-[var(--faint)]">
                {group.label}
              </div>
              {items.map((item) => {
                ...
                return (
                  ...
                  >
                    <Icon
                      className={`h-[18px] w-[18px] flex-shrink-0 ${active ? 'text-white' : 'text-[var(--txm)]'}`}
                    />
      ...
      <div className="sticky bottom-0 border-t border-[var(--bord2)] bg-[var(--sidebar)] px-3 py-3.5">
        <div className="mb-2.5 flex items-center gap-2.5 px-2">
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="block font-['Space_Mono'] text-[10px] uppercase tracking-[0.1em] text-[var(--txm)]">
              {roleLabel}
            </span>
          </span>
        </div>
