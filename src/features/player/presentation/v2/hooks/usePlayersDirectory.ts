...
export function usePlayersDirectory() {
  ...
  const reinstate = async (id: string) => { const player = players.find((row) => row.id === id); if (!player?.dropout || !window.confirm(`Reinstate ${getPlayerName(player)} on the roster they left?`)) return; try { await playerDirectoryApi.reinstate(id, player.dropout.rosterId); setPlayers((current) => current.map((row) => row.id === id ? { ...row, dropout: null } : row)); } catch { window.alert('Unable to reinstate this player. They may already be on another roster this season — open their profile for details.'); } };

  return { reinstate, players, seasons, currentSeason, scope, setScope, seasonId, setSeasonId, leagueSeasonId, setLeagueSeasonId, editions: currentSeason?.leagueSeasons ?? [], filters, updateFilter, resetFilters, teamOptions, base, filtered, visible, page: currentPage, pageCount, setPage, selected, setSelected, toggleSelection, toggleAll, sortKey, sortDirection, toggleSort, approve, toggleApproval, loading, error, load };
}
