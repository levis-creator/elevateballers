...
| Teams (directory)      | `src/pages/teams/index.astro` → `TeamsPage.astro`, legacy at `src/pages/teams-v1.astro` | `src/features/teams/presentation/v2/TeamsPage.astro` | ✅ built + CSS-isolated + live data (Clean Arch `teams` feature; directory is a React+Zustand island; reuses home Header/Footer/pill-styles) |
